...
- [ ] Test đỏ: `"mặc định xem team 1"`, `"chọn team 3 thì selectedTeam là 3"`,
      `"đổi team không đụng tới bản nháp và lịch sử hoàn tác"` (state của `useFormationStore` giữ nguyên).
- [ ] Xác nhận đỏ, viết store theo mẫu `pool-filter-store.ts`, xanh. Commit cùng Task 2.
...
- [ ] Test đỏ: `"hiện đủ 10 chip, mỗi chip có tên và số ô đã xếp trên 6"`,
      `"chip đang chọn mang aria-pressed=true, các chip khác false"`, `"bấm chip thì gọi onSelect với số
      team"`.
- [ ] Xác nhận đỏ. Component: `grid grid-cols-5 gap-1.5 md:hidden`; mỗi chip là `<button type="button"
...
- [ ] Xanh. Commit `feat(web): pick one team at a time in the team builder on a phone`.
...
- [ ] Test đỏ (mock `useDroppable`/`useDraggable` như test hiện có của team builder, hoặc bọc
...
- [ ] Xác nhận đỏ.
- [ ] `TeamColumn` nhận `className` và nối vào `Card`. `FormationGrid` đọc `useTeamViewStore`, tính
...
- [ ] Test đỏ: `"còn thay đổi chưa lưu thì dòng 'Lưu trước khi gửi' hiện bằng chữ"`;
      `"nút copy có tên truy cập đầy đủ, chữ ngắn cho điện thoại"`.
- [ ] Xác nhận đỏ.
- [ ] Toolbar: `flex flex-wrap items-center justify-end gap-2`, dưới `sm` hai nút `max-sm:flex-1`; nút
...
- [ ] `frontend.md` §6: "Unsaved work" (nút Hoàn tác), mục Xếp team trên điện thoại (một team, store
