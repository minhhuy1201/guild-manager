...
That is the whole of what the six *code* checks run, minus the two builds. Three more run on a pull request without a local equivalent — the SonarQube quality gate, dependency review and Trivy — for nine required checks in total; see [`production.md`](production.md) §6.
