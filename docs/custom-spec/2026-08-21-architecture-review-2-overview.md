...
| 10 | A4 | **Không nói ai sở hữu câu `'Tuần không hợp lệ.'`** Sau §4 câu đó phải xuất hiện ở hai tầng — Zod (`packages/shared`) và `parseWeekStart` (`apps/api`) — nên nó là một chuỗi lặp giữa hai package. | Hằng `INVALID_WEEK_MESSAGE` ở `packages/shared`, theo đúng khuôn `DEADLINE_CAP_MESSAGE`. *(**Đã đóng 2026-08-23** — chỉ spec thiếu: chỗ lặp giữa hai package tự hết cùng lỗi #4, nhưng quyền sở hữu thì chưa tài liệu nào phát biểu. A4 §4 nay có một đoạn riêng: câu đó thuộc `packages/shared` vì nó đi cùng luật validation (`architecture.md:316`), có **đúng một người đọc** là `weekStartQuerySchema`, vẫn giữ dạng hằng export để `apps/web` hiện lại được đúng câu chữ, và một `grep` ra hai package là dấu hiệu `parseWeekStart` lại biết chuyện của tầng biên. §1 đổi từ chuỗi trần sang tên hằng; doc comment của hằng trong `battle-session.schema.ts:27-33` bỏ câu *"Zod và API dùng chung"* đã sai; kế hoạch A4 điểm 3 và Task 2 cập nhật theo.)* |
| 11 | A5 §1 | **Chữ ký `VerifyToken` tự đánh bại mục đích của spec.** `VerifyToken = (token) => Promise<JwtPayload \| null>` (*"trả null thay vì ném"*) đẩy `.catch(() => null)` ngược về cả ba call site — đúng bản sao spec viết ra để xoá. | Đảo lại: `VerifyToken` **được phép ném**, `readToken` là chỗ duy nhất còn `.catch(() => null)`, khoá bằng `grep`. *(**Đã đóng 2026-08-23** — chỉ spec sai: bản hiện thực vốn theo kế hoạch (`read-bearer-token.ts:10` khai `Promise<JwtPayload>`, `:28` là dòng `.catch(() => null)` duy nhất trong `apps/api/src`, ba call site chỉ còn `(token) => this.jwt.verifyAsync<JwtPayload>(token)`). A5 §1 nay chép đúng chữ ký đó và nói vì sao đảo chiều là điều kiện để xoá được ba bản sao, kèm câu `grep -rn "catch(() => null)" apps/api/src` phải ra đúng một dòng; §4 ghi `readToken` là chỗ giữ `.catch`, bảng thay đổi thêm `VerifyToken`.)* |
| 12 | A5 | **Test §Kiểm thử yêu cầu không chạy được.** Spec đòi test 4 nhánh của `describeException`, nhưng hàm đang `private` trong `all-exceptions.filter.ts` và spec không nói phải mở nó. | `export` hàm, kèm một câu doc comment nói vì sao nó public. *(**Đã đóng 2026-08-23** — chỉ spec thiếu: bản hiện thực vốn theo kế hoạch (`all-exceptions.filter.ts:75` là `export function describeException`, doc comment `:73-74` nói vì sao, `all-exceptions.filter.spec.ts` 5 ca xanh). A5 §Kiểm thử nay có một đoạn riêng: vì sao `export` thắng đường test qua `filter.catch()` với `ArgumentsHost` giả, cái giá phải trả (hàm thành interface công khai nên phải mang doc comment tự giới hạn), và ghi rằng `common/index.ts` không cần thêm dòng vì đã `export *`. Bảng thay đổi thêm dòng `all-exceptions.filter.ts`; ô test đổi thành "4 nhánh + nhánh dự phòng" cho khớp ca thứ năm đang chạy — cùng lý do, doc comment trong code bỏ chữ "bốn nhánh" đếm sai.)* |
| 13 | A6 §4 | **Fix của spec vi phạm chính luật A3 vừa dựng.** Spec viết `tx.character.findMany` ngay trong `team-builder` — đi ngược `apps/api/docs/backend.md:120` (*"module đọc bảng của người khác thì gọi service của module đó"*). Đây là lỗi thứ hai của §4, tách khỏi lỗi #3 ở trên. | `CharactersService.listIds(client)` nhận transaction client của caller. Lỗi #3 được đóng sau đó (`ea8d0ed`): thêm nhánh bắt `P2003` → 409, race không còn thành 500. *(**Đã đóng 2026-08-23** — chỉ spec thiếu: code vốn theo kế hoạch (`team-builder.service.ts:187` gọi `this.characters.listIds(tx)`, `grep -rn "prisma\.character\|tx\.character" apps/api/src --exclude-dir=generated` chỉ còn `modules/characters/`). A6 §4 nay nói rõ ba điều spec bỏ trống: trích đúng luật binding kèm số dòng đã sửa (`backend.md:122-123`, không phải `:120`), **A3 §2 mới là chủ** của chữ ký `listIds` còn A6 chỉ là người gọi — để hai spec không cùng nửa-sở-hữu một hàm — và cạnh module `TeamBuilderModule → CharactersModule` là hệ quả bắt buộc, không sinh chu trình nên không cần `forwardRef()`. Bảng thay đổi thêm dòng `team-builder.module.ts`; §Rủi ro thêm cái giá của luật "một bảng, một chủ" và ai canh nó (`module-boundary.spec.ts`). Kế hoạch A6 điểm 1 hết là chỗ "khác spec".)* |
...
- **A4 lỗi #4 — đã đóng (2026-08-23).** `session-schedule.ts` trở lại thuần: bỏ import
  ...
  - `INVALID_WEEK_MESSAGE` giờ có đúng một người đọc (schema ở `packages/shared`), nên chỗ lặp giữa
    hai package mà mục #10 nêu cũng tự hết. Phần còn lại của #10 — **ai sở hữu câu đó** — được
    phát biểu ở A4 §4 ngày 2026-08-23; xem dòng #10 trong bảng trên.
