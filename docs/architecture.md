...
- Every other match is a scrim entered by an admin, who also sets its deadline — but **no later than
  10:00 VN on the match day**, and never after the match itself (`deadlineCapFor`). The form prefills
  12:00 VN the day before the match (`defaultDeadlineFor`), which is always within the cap. Admins can edit
  the open week and the next one (`getEditableWeeks`); past weeks are read-only.
- **Guild War's deadline is system-owned**: 12:00 Friday of its week (`guildWarDeadline`). Sending
