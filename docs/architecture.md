...
- Every other match is a scrim entered by an admin, who also sets its deadline — but **no later than
  11:00 VN on the match day**, and never after the match itself (`deadlineCapFor`). The form prefills
  that same cap, so a prefilled deadline is never rejected. Admins can edit the open week and the
  next one (`getEditableWeeks`); past weeks are read-only.
- **Guild War's deadline is system-owned**: 11:00 Saturday of its week, the morning of the match
  (`guildWarDeadline`). Sending `deadline` for a Guild War match is a 400, and `ensureGuildWar`
  rewrites the value on every read so a stale row corrects itself.
