...
| `BattleSession` | One match in a week. `weekStart` (Monday 00:00 VN) groups matches into weeks. Guild War uses the deterministic id `gw-<YYYY-MM-DD>` so it can be upserted idempotently; scrims get a `cuid()`. `deadline` is whatever the admin set — the backend does not clamp it. |
...
- Every other match is a scrim entered by an admin, who also sets its deadline. Admins can edit the
  open week and the next one (`getEditableWeeks`); past weeks are read-only.
